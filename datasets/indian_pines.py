...
class IndianPinesDataset(Dataset):
    def __init__(self, data_path, gt_path, split='train', 
        ...
        class_counts = np.zeros(17) # Index 0 is background
        ...
        for class_id in range(1, 17):
            ...
            # Record train counts to calculate sampling weights
        ...
        print(f"{split.upper()}: {len(self.coords)} labeled pixels active.")

        # --- PATCHING & WEIGHT LOGIC ---
        if self.use_patches:
            self.patches = []
            self.patch_weights = []
            
            # Calculate inverse weights for each class
            inv_class_weights = np.zeros(17)
            for c in range(1, 17):
                if class_counts[c] > 0:
                    inv_class_weights[c] = 1.0 / class_counts[c]
            
            # Normalize to avoid exploding weights
            ...
            for r in range(0, h - patch_size + 1, stride):
                for c in range(0, w - patch_size + 1, stride):
                    ...
                    # Only keep the patch if it contains labeled training pixels
                    if patch_mask.sum() > 0:
                        self.patches.append((r, c))
                        
                        # Weight = sum of the inverse class frequencies inside this specific patch
                        pixel_classes = patch_mask[patch_mask > 0].numpy()
                        patch_weight = np.sum(inv_class_weights[pixel_classes])
                        
                        # Add epsilon so no valid patch has an absolute zero chance of selection
